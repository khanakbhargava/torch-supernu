...
data = np.zeros([40, Np], dtype = float) #This array will store velocities and abundances for all Np particles
...
for i in range(64):
    ...
    for j in range(32):
       ...
       sabund = np.zeros([1,38], dtype = float) #Resetting sum of abundances for every cell
       x_left = x[j]
       x_right = x[j+1]
